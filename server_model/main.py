    ...
    try:
        ...
        while i < len(lines):
            if "PATIENT NAME" in lines[i]:
                details["PATIENT NAME"] = lines[i + 1].strip()
            elif ("Male" in lines[i] or "Female" in lines[i]) and i <= 10:
                details["SEX"] = lines[i].strip()
            elif "Years" in lines[i]:
                details["AGE"] = lines[i].strip()
            elif "Kgs" in lines[i]:
                details["WEIGHT"] = lines[i + 1].strip()

            elif "Haemoglobin" in lines[i]:
                details["TEST RESULTS"]["Haemoglobin"] = lines[i + 1].strip()

            elif "RBC Count" in lines[i]:
                details["TEST RESULTS"]["RBC Count"] = lines[i + 1].strip()

            elif "PCV" in lines[i]:
                details["TEST RESULTS"]["PCV"] = lines[i + 1].strip()

            elif "MCV" in lines[i]:
                details["TEST RESULTS"]["RBC INDICES"]["MCV"] = lines[i + 1].strip()
            elif "MCH." in lines[i] or "MCH " in lines[i]:
                details["TEST RESULTS"]["RBC INDICES"]["MCH"] = lines[i + 1].strip()
            elif "MCHC" in lines[i]:
                details["TEST RESULTS"]["RBC INDICES"]["MCHC"] = lines[i + 1].strip()
            elif "RDW." in lines[i] or "RDW " in lines[i]:
                details["TEST RESULTS"]["RBC INDICES"]["RDW"] = lines[i + 1].strip()

            elif "Total WBC Count" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Total WBC Count"] = lines[i + 1].strip()
            elif "Neutrophils" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Differential count"]["Neutrophils"] = lines[i + 1].strip()
            elif "Lymphocytes" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Differential count"]["Lymphocytes"] = lines[i + 1].strip()
            elif "Eosinophils" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Differential count"]["Eosinophils"] = lines[i + 1].strip()
            elif "Monocytes" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Differential count"]["Monocytes"] = lines[i + 1].strip()
            elif "Basophils" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Differential count"]["Basophils"] = lines[i + 1].strip()

            elif "Absolute Neutrophil Count" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Absolute Leucocyte Count"]["Absolute Neutrophil Count"] = lines[i + 1].strip()
            elif "Absolute Lymphocyte Count" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Absolute Leucocyte Count"]["Absolute Lymphocyte Count"] = lines[i + 1].strip()
            elif "Absolute Eosinophil Count" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Absolute Leucocyte Count"]["Absolute Eosinophil Count"] = lines[i + 1].strip()
            elif "Absolute Monocyte Count" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Absolute Leucocyte Count"]["Absolute Monocyte Count"] = lines[i + 1].strip()
            elif "Absolute Basophils Count" in lines[i]:
                details["TEST RESULTS"]["TOTAL WBC COUNT"]["Absolute Leucocyte Count"]["Absolute Basophils Count"] = lines[i + 1].strip()

            elif "Platelet Count" in lines[i]:
                details["TEST RESULTS"]["PLATELETS"]["Platelet Count"] = lines[i + 1].strip()
            elif "Platelets on Smear" in lines[i]:
                details["TEST RESULTS"]["PLATELETS"]["Platelets on Smear"] = lines[i + 1].strip()

            elif "RBC Morphology" in lines[i]:
                details["TEST RESULTS"]["PERIPHERAL BLOOD SMEAR"]["RBC"] = lines[i + 1].strip()
            elif "WBCs on PS" in lines[i]:
                details["TEST RESULTS"]["PERIPHERAL BLOOD SMEAR"]["WBC"] = lines[i + 1].strip()
            elif "Platelets" in lines[i]:
                details["TEST RESULTS"]["PERIPHERAL BLOOD SMEAR"]["Platelets"] = lines[i + 1].strip()
